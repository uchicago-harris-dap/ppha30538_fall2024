...
The native lecture slides and the PDF are posted at on the Canvas site. 
...
\vspace{2ex}
    ...
    -   self-sufficiency
    -   contemporary & realistic over perfect & clean
...
-   We will tell you about the five quirks now, so you can decide if this course is for you.
...
1.  Afternoon labs. 10 min quiz, 10 min mini lesson (most weeks), 60 min pair programming or group office hours (time to give and get help). 
    * Quizzes will usually cover the first lecture of the week they happen and the second lecture of the prior week. The reason for this is that some students attend lab before they attend the second lecture of the week.
...
-   Set up a time to talk with us (will be during lab)
...
-   This course is for you if you can devote substantial time to the class (quirk 1), want structure to learn open-source Python resources (quirk 2), are willing to struggle with the computer (quirk 3), and are willing to struggle with messy data (quirk 4).
...
To make sure ChatGPT doesn't server as a susbstitute for learning, you *may not* using ChatGPT for these components:

* In-class solo exercises + do-pair-shares (can use to look up unfamilar concepts)
...
\vspace{2ex}
    * Cite the individuals you collaborate directly with by including their names in the comments at the top of your assignment
* Online sources
\vspace{2ex}
    * Cite all code you use with a URL, even a one-line snippet. 
    * For AI tools provide the initial query string you used and an explanation of how you used the AI tool's response in writing your own response.
...
## Rhythms during lecture
* On the Github website, create a **fork** of the course [repo](https://github.com/uchicago-harris-dap/ppha30538_fall2024). 
* Make sure that you uncheck the box "copy the `main` branch only" because you will use there are two branches : `before_lecture` and `after_lecture`
\vspace{2ex}
    - `before_lecture` excludes the answers to in-class exercises and do-pair-shares
...
* Go to your new fork and clone that repo to your computer
* Before lecture: switch to the `before_lecture` branch and pull
* After lecture: switch to the `after_lecture` branch and pull
...
1. **Public Pset Questions**
\vspace{2ex}
    - Use the problem set-specific categories to keep questions organized and make sure you're not repeating a question. Best practice is to reference the problem set 
